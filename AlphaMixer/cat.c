...
int main() {
    ...
    char* data = aligned_alloc(32, outputWidth * outputHeight * sizeof(int32_t));

    XImage* image  = XCreateImage(d, defVis, dplanes, ZPixmap, 0, data, outputWidth, outputHeight, 16, 0);
        ...
        if (flag == 0) {
            ...
            for (int y = 0; y < tableHeight; y++) {
                for (int x = 0; x < tableWidth; x++) {
                    int32_t catPixel   = *((int32_t*)catBuffer + y * tableWidth + x);
                    int32_t tablePixel = *((int32_t*)tableBuffer + y * tableWidth + x);

                    uint8_t catRed   = (catPixel & 0xff000000);
                    uint8_t catGreen = (catPixel & 0x00ff0000);
                    uint8_t catBlue  = (catPixel & 0x0000ff00);
                    uint8_t trans    = (catPixel & 0x000000ff);

                    uint8_t tableRed   = (tablePixel & 0xff000000);
                    uint8_t tableGreen = (tablePixel & 0x00ff0000);
                    uint8_t tableBlue  = (tablePixel & 0x0000ff00);

                    *((int32_t*)image->data) = (catRed * trans + tableRed * (255 - trans))  << 24 + 
                                               (catGreen * trans + tableGreen * (255 - trans)) << 16 +
                                               (catGreen * trans + tableGreen * (255 - trans)) << 8  +
                                               0xff;
                }
            }
        }
